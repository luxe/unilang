...
## Overview
Unilang is a **universal programming language (UPL)**.  A UPL was not formally defined prior to the creation of this document.  As an informal definition, we envision UPLs as language abstractions over **general-purpose programming languages (GPLs)**.  The reason this is necessary will become evident in our analysis of GPLs.  Despite their name, GPLs become commonly associated with specific niches across different programming domains.  Due to the differences between these languages (both internal and external to their design), evaluations lead to different languages being chosen depending on the domain being operated under.  A cross-domain problem becomes solved through a mixture of languages.  Simply put, GPLs have not proven general enough as a solution to all problems.  Unilang's stance is that the amalgamation of different languages could be an implementation detail below a more conceptually pure representation of the full problem ontology.  Unilang takes a novel approach to language's role in facilitating software.  Since the design and implementation of Unilang is built with particular language philosophies, this overview will discuss these ideas first.  Unilang will then be described in the context of software engineering.  
