...
## Overview
Unilang is a **universal programming language (UPL)**.  A UPL has not been formally defined prior to the creation of this document.  We envision UPLs as language abstractions over **general-purpose programming languages (GPLs)**.  Despite their name, GPLs become commonly connected to specific niches across different programming domains.  Due to the differences between languages (both internal and external to their design), evaluations lead to different languages being chosen depending on the domain being operated under.  A cross-domain problem becomes solved with a mixture of languages.  Unilang's stance is that the amalgamation of different languages should be the implementation detail below a more conceptually pure representation of the full problem ontology.  Unilang takes a novel approach to language's role in facilitating software.  Since the design and implementation of Unilang is built with particular language philosophies, this overview will discuss these ideas first.  Unilang will then be described in the context of software engineering.  
