...
def static_tree_structs(name):
    ...
    native.genrule(
        ...
        cmd = "$(location //code/tools/transcompilers/tree_hcp/static_tree_structs:static_tree_structs) -i $(SRCS) --name {}  -o $(@D)".format(name),
    )

    #compile cpp object
    cpp_object(name + "_static_tree_structs")