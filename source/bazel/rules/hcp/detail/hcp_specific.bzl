...
def hcp_specific(name, hcp_name, hpp_name, cpp_name, deps = [], data = []):
    ...
    native.genrule(
        ...
        cmd = "$(location //code/programs/transcompilers/hcp-compiler/official:hcp-compiler) $(SRCS) -o $(@D) --output-base-name {}".format(name),
    )
