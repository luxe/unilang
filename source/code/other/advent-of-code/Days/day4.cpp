#include "day4.hpp"
 ...
 std::string Day4::As_Shifted(std::string name, int shift_amount){
    for (auto & it: name){
 ...
 void Day4::CycleThroughLowercase(char & ch, int skipcount){
 ...
 Encryption Day4::Parse(std::string str){
    Encryption line;
 ...
 void Day4::Decide_Valids_And_Invalids(std::vector<Encryption> const& lines,std::vector<Encryption> & valid, std::vector<Encryption> & invalid){
 ...
 bool Day4::Valid(Encryption const& line){
    std::map<char,int> m;
