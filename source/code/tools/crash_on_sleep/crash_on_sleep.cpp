...
void read_block(pid_t pid, void * dst, void * src, size_t len) {
    for (size_t i = 0; i < len; i += sizeof(void *)) {
        *(void **)(static_cast<char*>(dst) + i) = (void *)ptrace(PTRACE_PEEKDATA, pid, static_cast<char*>(src) + i, NULL); // XXX
    }
}

void write_block(pid_t pid, void * src, void * dst, size_t len) {
    for (size_t i = 0; i < len; i += sizeof(void *)) {
        ptrace(PTRACE_POKEDATA, pid, static_cast<char*>(dst) + i, *(void **)(static_cast<char*>(src) + i)); // XXX
    }
}
