...
{
  stack_node *new_var = (stack_node *)malloc(sizeof(stack_node));
  if (!new_var) {
    fprintf(stderr, "out of memory!\n");
    exit(-1);
  }
  new_var->size = size;
  new_var->next = poles[which].head;
  poles[which].head = new_var;
  poles[which].depth++;
}
...
mpush(int t, int f)
{
  move_node *new_var = (move_node *)malloc(sizeof(move_node));
  if (!new_var) {
    fprintf(stderr, "Out of memory!\n");
    exit(-1);
  }
  ...
  new_var->f = f;
  new_var->next = NULL;
  new_var->prev = moves.tail;
  if (moves.tail)
    moves.tail->next = new_var;
  moves.tail = new_var;
  if (!moves.head)
