...
void flat_face(float ir, float var_or, float wd);
void draw_inside(float w1, float w2, float rad);
void draw_outside(float w1, float w2, float rad);
void tooth_side(int nt, float ir, float var_or, float tp, float tip, float wd);

int circle_subdiv;

void
gear(int nt, float wd, float ir, float var_or, float tp, float tip, int ns, Profile * ip)
{
  /**
   * nt - number of teeth 
  ...
  /* --- tooth side faces --- */
  tooth_side(nt, ir, var_or, tp, tip, wd);

  /* --- tooth hill surface --- */
}

void 
tooth_side(int nt, float ir, float var_or, float tp, float tip, float wd)
{
  ...
  var_or = var_or * ir;         /* or is really a ratio of ir */
  for (i = 0; i < 2.0 * M_PI - end / 4.0; i += end) {
    ...
    x[2] = var_or * fcos(i + end * (0.5 - tip / 2));
    y[2] = var_or * fsin(i + end * (0.5 - tip / 2));
    x[3] = var_or * fcos(i + end * (0.5 + tip / 2));
    y[3] = var_or * fsin(i + end * (0.5 + tip / 2));
...
void 
flat_face(float ir, float var_or, float wd)
{
  ...
  for (w = wd / 2; w > -wd; w -= wd) {
    ...
    if (ir == 0.0) {
      ...
      glVertex3f(var_or, 0.0, w);
      for (i = 1; i < circle_subdiv; i++) {
        glVertex3f(fcos(2.0 * M_PI * i / circle_subdiv) * var_or,
          fsin(2.0 * M_PI * i / circle_subdiv) * var_or,
          w);
      }
      glVertex3f(var_or, 0.0, w);
      glEnd();
