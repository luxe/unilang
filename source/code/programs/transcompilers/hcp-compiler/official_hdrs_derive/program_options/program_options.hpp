...
class Program_Options: public Program_Options_Default{
	...
	/*  */ std::vector<std::string> Input_Files()const; //the name of the hcp file to compile;
        ...
        std::string Output_Base_Name() const;

	//deps & headers
	std::vector<std::string> Interface_Deps() const;
	std::vector<std::string> Implementation_Deps() const;

        // headers (that don't match deps format)
	std::vector<std::string> Interface_Hdrs() const;
	std::vector<std::string> Implementation_Hdrs() const;
