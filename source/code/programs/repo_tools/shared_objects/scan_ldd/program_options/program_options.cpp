...
boost::program_options::options_description Program_Options::Get_Options_Description(void){
	...
	//Program Description
	options_description desc("find a shared object based on a target");
    ...
    ("dir",value<std::string>(),"the directory to search in")
    ("shared_object",value<std::string>(),"the shared object to find")
...
std::string Program_Options::Shared_Object() const{
	std::string data;
	if (vm.count("shared_object")){
		data = vm["shared_object"].as<std::string>();
	}
