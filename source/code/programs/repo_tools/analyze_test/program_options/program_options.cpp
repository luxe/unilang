...
boost::program_options::options_description Program_Options::Get_Options_Description(void){
	...
	//Program Description
	options_description desc("Analyze a unit test");
    ...
    ("targets",value<std::vector<std::string>>(),"the target to analyze")
...
std::vector<std::string> Program_Options::Targets() const{
	std::vector<std::string> data;
	if (vm.count("targets")){
		data = vm["targets"].as<std::vector<std::string>>();
	}
