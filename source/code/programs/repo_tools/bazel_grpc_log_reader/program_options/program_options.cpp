...
boost::program_options::options_description Program_Options::Get_Options_Description(void){
	...
	//Program Description
	options_description desc("parse bazel's grpc log");
	...
	("path",value<std::string>(),"the path to the grpc log")
...
void Program_Options::Process_Immediate_Options( boost::program_options::options_description const& desc){
	...
	if (vm.count("version")){
		std::cout << "\nThis is version " << "1" << " of grpc log parser.\n\n";
		exit(EXIT_SUCCESS);
	}
...
std::string Program_Options::Path() const{
	std::string data;
	if (vm.count("path")){
		data = vm["path"].as<std::string>();
	}
