...
boost::program_options::options_description Program_Options::Get_Options_Description(void){
	...
	//Program Description
	options_description desc("analyze build plan dependencies");
    ...
    ("input",value<std::string>(),"build plan file")
...
std::string Program_Options::Input() const{
	std::string data;
	if (vm.count("input")){
		data = vm["input"].as<std::string>();
	}
