...
xcb_char2b_t *build_chars(const char *str, size_t length) {
  ...
  int i;
  xcb_char2b_t *ret = static_cast<xcb_char2b_t*>(malloc(length * sizeof(xcb_char2b_t)));
  if (!ret)
    return NULL;
...
button_t create_button(char *label, int x, int y, fptr_t *click_callback) {
  ...
  b = (button_t){window_id, "", static_cast<char>(origin), static_cast<char>(width), static_cast<char>(height), 0, {0,0},{static_cast<uint32_t>(x),static_cast<uint32_t>(y)}, click_callback};
...
void drop_paint(xcb_gcontext_t cgc, button_t *b) {
  ...
  xcb_poly_fill_rectangle(c, back_pixmap, cgc, 1,
              (xcb_rectangle_t[]){{ static_cast<int16_t>(b->origin[0]-5), static_cast<int16_t>(b->origin[1]-5), 5, 5}});
