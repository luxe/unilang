...
combine(GLdouble coords[3], void *d[4], GLfloat w[4], void **dataOut)
{
  ...
  if (combineNext >= combineListSize) {
    ...
    new_var = &combineList[combineNext * 2];
  }
...
processFeedback(GLint size, GLfloat * buffer)
{
  ...
  if (combineNext > combineListSize) {
    ...
    combineList = (float*)realloc(combineList, sizeof(GLfloat) * 2 * combineListSize);
  }
  combineNext = 0;
...
doFeedback:
  ...
  if (bufferSize < probableSize) {
    ...
    feedbackBuffer = (float*)realloc(feedbackBuffer, bufferSize * sizeof(GLfloat));
  }
