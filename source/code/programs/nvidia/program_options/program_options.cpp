...
boost::program_options::options_description Program_Options::Get_Options_Description(void){
	...
	//Program Description
	options_description desc("prune architectures from nvidia cuda objects");
    ...
    ("file",value<std::string>(),"the file to perform pruning on (typically a .tgz)")
    ("keep",value<std::vector<std::string>>(),"the cuda archs to keep")
    ("skip",value<std::vector<std::string>>(),"the cuda archs to skip entirely (pretend they don't exist (this is becaus nvprune can segfault)")
...
std::vector<std::string> Program_Options::Keep() const{
	std::vector<std::string> data;
	if (vm.count("keep")){
		data = vm["keep"].as<std::vector<std::string>>();
	}

	return data;
}
std::vector<std::string> Program_Options::Skip() const{
	std::vector<std::string> data;
	if (vm.count("skip")){
		data = vm["skip"].as<std::vector<std::string>>();
	}
