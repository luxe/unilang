...
class Program_Options: public Program_Options_Default{
	...
	//| USER FLAGS                                               |
	//+----------------------------------------------------------+
	/* "" */ std::string Program_Name()const; //the name of the program being installed. If no name is given, the makefile assumes a generative dependency;
	/* true */ bool Move()const; //the binary being built by the Makefile, should not be moved.;
	/*  */ std::vector<std::string> Additional_Libraries()const; //add additional libraries to be linked against;
