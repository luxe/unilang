...
//helper functions for group boolean testing
template <typename... Args>
inline bool All_True(Args&&... args){
    return true == All_Of(std::forward<Args>(args)...);
}
template <typename... Args>
inline bool All_False(Args&&... args){
    return false == All_Of(std::forward<Args>(args)...);
}
template <typename... Args>
inline bool Any_True(Args&&... args){
    return true == Any_Of(std::forward<Args>(args)...);
}
template <typename... Args>
inline bool Any_False(Args&&... args){
    return false == Any_Of(std::forward<Args>(args)...);
}
template <typename... Args>
inline bool None_True(Args&&... args){
    return true == None_Of(std::forward<Args>(args)...);
}
template <typename... Args>
inline bool None_False(Args&&... args){
    return false == None_Of(std::forward<Args>(args)...);
}

//template<>
//unsigned int Count_Trues() {
//  return 0;
//}
template<typename T>
unsigned int Count_Trues(T&& v) {
  return v ? 1 : 0;
}

template<typename T, typename... Args>
unsigned int Count_Trues(T&& first, Args&&... args) {
  return first ? 1 + Count_Trues(std::forward<Args>(args)...) : 0 + Count_Trues(std::forward<Args>(args)...);
}
//template<>
//unsigned int Count_Falses() {
//  return 0;
//}
template<typename T>
unsigned int Count_Falses(T&& v) {
  return v ? 0 : 1;
}
template<typename T, typename... Args>
unsigned int Count_Falses(T&& first, Args&&... args) {
  return first ? 0 + Count_Falses(std::forward<Args>(args)...) : 1 + Count_Falses(std::forward<Args>(args)...);
}

template <typename... Args>
bool More_Than_One_Is_True(Args&&... args){
  return Count_Trues(std::forward<Args>(args)...) > 1;
}

template <typename... Args>
bool More_Than_One_Is_False(Args&&... args){
  return Count_Falses(std::forward<Args>(args)...) > 1;
}

template <typename... Args>
bool Only_One_Is_True(Args&&... args){
  return Count_Trues(std::forward<Args>(args)...) == 1;
}

template <typename... Args>
bool Only_One_Is_False(Args&&... args){
  return Count_Falses(std::forward<Args>(args)...) == 1;
}
